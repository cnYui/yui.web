...
function createId(prefix, random = Math.random) {
    return `${prefix}${Date.now().toString().slice(-8)}${Math.floor(random() * 9000 + 1000)}`;
}
...
`).run(breakdown.inputTokens, breakdown.cachedTokens, breakdown.cacheHitInputTokens, breakdown.cacheMissInputTokens, row.request_id);
            ...
            const existing = db.prepare('SELECT status, charge_nanos FROM api_charge_records WHERE usage_event_id = ?').get(row.request_id);
