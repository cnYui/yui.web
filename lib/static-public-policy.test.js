...
test('公网静态白名单允许用户页面和资源', () => {
    ...
    assert.equal(isAllowedPublicStaticPath('/files/WU_JIANXIANG_resume.pdf'), true);
});
...
test('缓存策略区分 HTML 和可缓存静态资源', () => {
    ...
    assert.match(cacheControlForStaticPath('/files/WU_JIANXIANG_resume.pdf'), /max-age=604800/);
});
