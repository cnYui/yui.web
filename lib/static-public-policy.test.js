...
test('公网静态白名单允许用户页面和资源', () => {
    ...
    assert.equal(isAllowedPublicStaticPath('/files/webxr-generic-hand-right.glb'), true);
});
...
test('公网静态白名单屏蔽源码、脚本、测试和协作文档', () => {
    ...
    // 简历 PDF 只能经密码校验接口取回，直连静态路径必须 404。
    assert.equal(isAllowedPublicStaticPath('/files/WU_JIANXIANG_resume.pdf'), false);
    assert.equal(isAllowedPublicStaticPath('/package.json'), false);
    assert.equal(isAllowedPublicStaticPath('/AGENTS.md'), false);
});

test('动漫、音乐页面及其专属图片暂时不对公网开放', () => {
    assert.equal(isAllowedPublicStaticPath('/anime/'), false);
    assert.equal(isAllowedPublicStaticPath('/anime/index.html'), false);
    assert.equal(isAllowedPublicStaticPath('/music/'), false);
    assert.equal(isAllowedPublicStaticPath('/images/optimized/animate/image copy 10.webp'), false);
    assert.equal(isAllowedPublicStaticPath('/images/optimized/music_pic/a.webp'), false);
    assert.equal(isAllowedPublicStaticPath('/images/animate/image.png'), false);
    assert.equal(isAllowedPublicStaticPath('/images/optimized/travel/nara-deer-2026.webp'), true);
    assert.equal(isAllowedPublicStaticPath('/images/hackathon/osaka-rokid-mini-hackathon-2026.webp'), true);
});
...
test('缓存策略区分 HTML 和可缓存静态资源', () => {
    ...
    assert.match(cacheControlForStaticPath('/files/handbook.pdf'), /max-age=604800/);
});
