...
const TechBlog = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // 当用户开始滚动时隐藏提示
      if (scrollTop > 10 && showScrollHint) {
        setShowScrollHint(false);
      }

      // 计算ProfileCard的同步滚动偏移量
      // 当右侧内容滚动时，左侧ProfileCard向上移动
      // 最大向上移动距离为150px，根据滚动进度线性映射�?      // 使用缓动函数优化滚动体验
      ...
      if (!isScrolling) {
        // 检测是否滚动到底部（95%以上认为是底部）�?5%以上认为是底部）
  ...
  }, [isScrolling, hasScrolledToBottom, showScrollHint]);
      ...
      <div 
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '2rem' }}>
            ...
            >
              <div style={{ lineHeight: '1.2', color: '#e8e8e8' }}>
                对新兴技术保持着永不熄灭的好奇心，我喜欢动手实践，从Touch Design到智能的AI应用，不断挑战自己的技术边界。每一次技术突破都是对未知领域的勇敢探索，每一行代码都承载着对完美的不懈追求。
              </div>
            </ScrollReveal>
          </div>
          ...
          <div style={{ marginBottom: '2rem' }}>
            ...
            >
              <div style={{ lineHeight: '1.2', color: '#d0d0d0' }}>
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  <strong style={{ color: '#ff6b6b' }}>开发环境：</strong>Windsurf、Cursor、Trae、CodeBuddy、kiro、Qoder、Antigravity — 现代化的编程体验
                </div>
              </div>
            </ScrollReveal>
          </div>
